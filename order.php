...
EOV;
    ...
    if ($_REQUEST['payment_redirect'] and $data['code'] == 'ok' and !$data['async_save']) {
        ...
        if (isset($_COOKIE["FBpixel"])) {
            $fb_pixel_id = $_COOKIE["FBpixel"];
        } else {
            $fb_pixel_id = null;
        }
        $payment_resp = Get_Payment_Resp($_REQUEST, $data['esub'], $data['goods_id'], $fb_pixel_id);
...
if ($data['code'] == 'ok') {
    // Don't render pixel if order is_double or already fired for this order
    $pixel_cookie = 'pixel_fired_' . $data['order_id'];
    if (!$data['is_double'] && !isset($_COOKIE[$pixel_cookie])) {
        $pixel_code = ACLandingConfig::PIXEL_CODE;
        setcookie($pixel_cookie, '1', time() + 21600, '/');
    }
