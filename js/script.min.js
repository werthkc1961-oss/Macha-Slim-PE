...
class GenerateData {
  constructor(selector, mathDay) {
    ...
    this._monthName = ["Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno", "Luglio", "Agosto", "Settembre", "Ottobre", "Novembre", "Dicembre"]
  }
  getData() {
  ...
  getMoth() {
    ...
    if (month === 0)
      fMonth = "gennaio";
    if (month === 1) fMonth = "febbraio";
    if (month === 2) fMonth = "marzo";
    if (month === 3) fMonth = "aprile";
    if (month === 4) fMonth = "maggio";
    if (month === 5) fMonth = "giugno";
    if (month === 6) fMonth = "luglio";
    if (month === 7) fMonth = "agosto";
    if (month === 8) fMonth = "settembre";
    if (month === 9) fMonth = "ottobre";
    if (month === 10) fMonth = "novembre";
    if (month === 11) fMonth = "dicembre";
    this.#render(fMonth)
  }
  getFull() {
...
class GenerateCta {
  ...
  #door(options) {
    ...
    const doorItem = this.$el.querySelectorAll(".door__item");
    const doorItemImages = this.$el.querySelectorAll(".door__item-img");
    doorItem.forEach((door) => {
      door.addEventListener("click", this)
    });
    // Check if site opened on the phone
    if(window.innerWidth <= 768) {
      // Automatic door open every 1.2 seconds
      var currentIndex = 0;
      this.$doorIntervalId = setInterval(() => {
        currentIndex = (currentIndex + 1) % 3;
        this.#doorOpener(doorItemImages, currentIndex);
      }, 1200);
    }
  ...
  #discount(event) {
    ...
    if (event.currentTarget.dataset.index === "0") {
      ...
      discount2.innerHTML = "20%";
      discount3.innerHTML = "10%"
    }
    if (event.currentTarget.dataset.index === "1") {
      discount1.innerHTML = "20%";
      discount2.innerHTML = "50%";
      discount3.innerHTML = "10%"
    }
  ...
  #doorOpener(doors, index) {
    const previousIndex = (index - 1 + 3) % 3;
    doors[previousIndex].classList.remove('hovered');
    doors[index].classList.add('hovered');
  }
  #stopDoorOpener() {
    clearInterval(this.$doorIntervalId);
  }
}
class SctollTo {
  constructor(obj) {
